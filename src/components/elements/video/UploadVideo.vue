                            ...
                            <div class="rnform-group">
                                <el-select v-model="form.activityID" placeholder="请选择视频活动">
                                    <el-option v-for="actTag in actTagList" :key="actTag.tid"
                                        v-bind:label="actTag.tagName" v-bind:value="actTag.tid"></el-option>
                                </el-select>
                            </div>
                        </el-form-item>
                    </div>
                        ...
                        <el-form-item label="视频标签" prop="tags">
                            <div class="rnform-group">
                                <el-checkbox-group v-model="videoTags">
                                    <el-checkbox-button v-for="tag in tagList" :key="tag.tid" v-bind:label="tag.tagName"
                    ...
                    <div class="col-lg-12">
                        <div class="blog-btn">
                            <el-button class="btn-default btn-large" :plain="true"
                                @click="submit('form')">确认投稿</el-button>
                        </div>
                    </div>
                </div>
            </el-form>
        ...
        submit(name) {
            this.$refs[name].validate((valid) => {
                if (valid) {
                    ...
                    this.$axios.post("/video", formItems).then(res => {
                        console.log(res);
                        if (res.data.state == 0) {
                            this.$notify({
                                type: 'success',
                            ...
                            this.$notify({
                                type: 'success',
                                title: '请前往稿件管理标签页查看',
                                message: '视频信息存储成功，无需重复存储'
                            });
                            // 跳转去视频管理页面
                            setTimeout(() => {
                                this.$router.go(0);
                            }, '2000');
                            // 想设置自动切换，切不了，毕竟不是elementui那样方便，有api文档可以查
                            // this.$refs.activeFilter = '稿件管理';
                            // this.$refs.activeTab = 1;
                        } else {
                            this.$notify.info({

    ...
    <el-form class="contact-form-1" ref="formValidate" :model="formValidate" :rules="ruleValidate" action="">
        <el-form-item label="用户名：" prop="username">
            <div class="form-group">
                <el-input type="text" name="username" placeholder="输入您的用户名" v-model="formValidate.username"
                    required />
            </div>
        </el-form-item>
        <el-form-item label="昵称：" prop="nickname">
            <div class="form-group">
                <el-input type="text" name="nickname" placeholder="输入您的昵称，不输入则会随机生成" v-model="formValidate.nickname"
                    required />
            </div>
        </el-form-item>
        <el-form-item label="邮箱：" prop="email">
            <div class="form-group">
                <el-input type="email" name="email" placeholder="输入您的邮箱地址" v-model="formValidate.email" required />
            </div>
        </el-form-item>

        <el-form-item label="手机号：" prop="phone">
            <div class="form-group">
                <el-input type="text" name="phone" placeholder="输入您的电话号码" v-model.number="formValidate.phone"
                    required />
            </div>
        </el-form-item>
        ...
        <el-form-item label="密码：" prop="password">
            <div class="form-group">
                <el-input type="password" name="password" placeholder="输入您的密码" v-model="formValidate.password"
                    required />
            </div>
        </el-form-item>
        ...
        <el-form-item label="确认密码：" prop="againPassword">
            <div class="form-group">
                <el-input type="password" name="confirm_password" placeholder="请重新输入您的密码"
...
export default {
    ...
    data() {
        ...
        return {
            showResult: false,
            result: `<p class="success-message">账号注册成功，请验证邮箱！</p>`,
            changeFlag: 0,
            changeAgainFlag: 0,
            formValidate: {
                username: '',
        ...
        sendEmail(name) {
            this.$refs[name].validate((valid) => {
                if (valid) {
                    ...
                    this.$axios.post("/user/register", formItems).then(res => {
                        console.log(res);
                        if (res.data.state == 0) {
                            this.$notify({
                                type: 'success',
